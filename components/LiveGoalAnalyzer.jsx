...
const DEFAULT_WEIGHTS = { sot: 40, xg: 30, corners: 20, recent: 10 };
const CONVERGENCE_STRONG_SHARE = 60;
...
function normalizeLiveMatch(raw) {
  ...
  return {
    id: raw.id, home: raw.home_team, away: raw.away_team, competition: raw.competition,
    minute: raw.minute ?? 0, scoreHome: raw.score_home ?? 0, scoreAway: raw.score_away ?? 0,
    ...
    missing, bias: 0.5, history: [], finished: raw.status === "finished",
  };
}
...
function makeInitialMatch(id, [home, away, competition]) {
  ...
  return {
    id, home, away, competition, minute, scoreHome: 0, scoreAway: 0,
    ...
    fouls: Math.floor(Math.random() * 6), yellow: 0, red: 0,
    missing: [], bias, history: [], lastAlertScore: 0, finished: false,
  };
}
...
function computePressure(m, weights) {
  ...
  return {
    score: Math.round(adjustedScore), dominant,
    homeShare: Math.round(dominant === "home" ? adjustedScore : 100 - adjustedScore),
    recentShare: Math.round(recentShare), convergence,
...
function tickMatch(m) {
  ...
  const homeIsActing = Math.random() < driftedBias;
  if (Math.random() < 0.75) {
    if (homeIsActing) {
      next.shotsHome += 1;
  ...
  if (Math.random() < 0.006) next.yellow += 1;
  ...
  next.history = [...m.history, {
    minute: next.minute, homeShare: tempPressure.homeShare,
    ...
    convergenceCount: tempPressure.convergence.count, goal,
  }];
  return next;
}

function StatBar({ label, home, away, format = (x) => x }) {
  const total = home + away || 1;
  const homePct = (home / total) * 100;
  return (
    <div className="mb-3">
      <div className="flex justify-between text-xs text-slate-400 mb-1 font-mono">
        <span>{format(home)}</span><span className="text-slate-500">{label}</span><span>{format(away)}</span>
      </div>
...
function SignalBadge({ score }) {
  ...
  return <span className={`inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full border ${t.badge}`}>{sig.emoji} {sig.label}</span>;
}
  ...
  return (
    <div>
      <div className="flex justify-between text-xs text-slate-400 mb-1">
        <span>Pression offensive</span><span className="font-mono text-slate-200">{score}/100</span>
      </div>
...
function Dashboard({ matches, pressures, filters, setFilters, onOpen }) {
  ...
  return (
    <div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
        {filtered.map((m) => (<MatchCard key={m.id} m={m} pressure={pressures[m.id]} onOpen={onOpen} />))}
        {filtered.length === 0 && (<div className="col-span-full text-center text-slate-500 py-12 text-sm">Aucun match ne correspond à ces filtres.</div>)}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-wrap gap-2 items-center">
      ...
      <select value={filters.minSot} onChange={(e) => set("minSot", Number(e.target.value))} className="text-xs bg-slate-900 border border-slate-800 rounded-full px-3 py-1.5 text-slate-300">
        <option value={0}>Tirs cadrés ≥ 0</option><option value={3}>≥ 3</option><option value={5}>≥ 5</option><option value={7}>≥ 7</option><option value={10}>≥ 10</option>
      </select>
    </div>
  );
}
...
function MatchDetail({ m, pressure, onBack }) {
  const chartData = m.history.map(h => ({ minute: h.minute, "Domicile": h.homeShare, "Extérieur": 100 - h.homeShare }));
  return (
    <div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 mb-4">
        <PressureBar score={pressure.score} />
        {pressure.missingLabels.length > 0 && (<p className="text-xs text-amber-400 mt-3">⚠️ Certaines statistiques ne sont pas disponibles : {pressure.missingLabels.join(", ")}.</p>)}
      </div>
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 mb-4">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 mb-4">
        ...
        <ul className="text-sm text-slate-300 space-y-1">{convergenceReasons(m, pressure).map((r, i) => <li key={i}>{r}</li>)}</ul>
      </div>
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 mb-4">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
        <h3 className="text-sm font-semibold text-slate-200 mb-3">Évolution de la pression</h3>
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <label className="flex items-center gap-2 text-sm text-slate-400">
          <input type="checkbox" checked={alertsEnabled} onChange={(e) => setAlertsEnabled(e.target.checked)} />Alertes activées
        </label>
      </div>
      <div className="space-y-2">
...
function HistoryView({ matches }) {
  const rows = matches.flatMap((m) => m.history.slice(-6).map((h) => ({ match: `${m.home} – ${m.away}`, minute: h.minute, homeShare: h.homeShare, sot: `${h.sotHome}-${h.sotAway}`, goal: h.goal })));
  return (
    <div>
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} className="border-b border-slate-900 text-slate-300">
                <td className="py-2 pr-4">{r.match}</td><td className="py-2 pr-4 font-mono">{r.minute}'</td><td className="py-2 pr-4 font-mono">{r.homeShare}%</td><td className="py-2 pr-4 font-mono">{r.sot}</td><td className="py-2">{r.goal ? "⚽" : "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function BacktestView({ matches, threshold, setThreshold }) {
  const results = useMemo(() => {
    const allSnaps = matches.flatMap((m) => m.history.map((h) => ({ ...h, full: m.history })));
    ...
    hits.forEach((s) => {
      ...
      if (future.length > 0) {
        ...
        delays.push(delay);
        if (delay <= 5) within5++; if (delay <= 10) within10++; if (delay <= 15) within15++;
      }
    });
    const n = hits.length || 1;
    return { n: hits.length, pct5: Math.round((within5/n)*100), pct10: Math.round((within10/n)*100), pct15: Math.round((within15/n)*100), avgDelay: delays.length ? (delays.reduce((a,b)=>a+b,0)/delays.length).toFixed(1) : "—" };
  }, [matches, threshold]);
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        {[60, 70, 80, 90].map((t) => (<button key={t} onClick={() => setThreshold(t)} className={`text-xs px-3 py-1 rounded-full border ${threshold === t ? "bg-orange-500/15 border-orange-500/40 text-orange-300" : "bg-slate-900 border-slate-800 text-slate-400"}`}>{t}</button>))}
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        <SummaryTile emoji="📈" label="Situations" value={results.n} />
...
function SettingsView({ weights, setWeights }) {
  ...
  const labels = { sot: "🎯 Tirs cadrés", xg: "⚽ xG", corners: "🚩 Corners", recent: "📈 Dynamique récente" };
  return (
    <div>
      <h2 className="text-lg font-bold text-slate-100 mb-2">Paramètres</h2>
      <div className="space-y-4 max-w-md">
        {Object.keys(weights).map((k) => (
          <div key={k}>
            <div className="flex justify-between text-sm text-slate-300 mb-1"><span>{labels[k]}</span><span className="font-mono">{weights[k]}%</span></div>
            <input type="range" min={0} max={60} value={weights[k]} onChange={(e) => set(k, Number(e.target.value))} className="w-full accent-orange-500" />
          </div>
        ))}
      </div>
      <div className={`mt-4 text-sm font-mono ${total === 100 ? "text-emerald-400" : "text-red-400"}`}>Total : {total}%</div>
    </div>
  );
}
...
function Methodology() {
  return (
    <div className="text-sm text-slate-300 space-y-4">
      ...
      <p>Tirs cadrés (40%), xG (30%) et corners (20%) forment le cœur du moteur, complétés par la dynamique récente (10%). Une statistique indisponible est exclue du calcul, jamais inventée.</p>
      <p>Le moteur vérifie la convergence des indicateurs plutôt que leur simple volume : beaucoup de corners avec peu de tirs cadrés ne donne pas un signal très fort.</p>
      <p>Un score élevé signifie une domination statistique, pas une probabilité garantie de but.</p>
    </div>
  );
}
...
export default function LiveGoalAnalyzer() {
    ...
    const poll = async () => {
      ...
      try {
        ...
        setMatches((prev) => fresh.map((f) => {
          ...
          withHistory.history = [...withHistory.history, { minute: withHistory.minute, homeShare: p.homeShare, sotHome: withHistory.sotHome, sotAway: withHistory.sotAway, cornersHome: withHistory.cornersHome, cornersAway: withHistory.cornersAway, xgHome: withHistory.xgHome, xgAway: withHistory.xgAway, scoreHome: withHistory.scoreHome, scoreAway: withHistory.scoreAway, convergenceCount: p.convergence.count, goal: null }];
          if (before) {
            const prevScore = computePressure(before, weights).score;
        ...
        setLiveStatus("ok"); setLiveError(null); setLastUpdated(Date.now());
      } catch (e) {
        if (cancelled) return;
        setLiveStatus("error"); setLiveError(e.message || "Erreur inconnue");
      }
    };
    poll();
  ...
  const nav = [["dashboard", "Dashboard"], ["live", "Matchs Live"], ["top", "Analyse"], ["history", "Historique"], ["backtest", "Statistiques"], ["settings", "Paramètres"], ["methodology", "Méthodologie"]];
      ...
      <header className="border-b border-slate-800 sticky top-0 bg-slate-950/95 backdrop-blur z-10">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-2">
            ...
            {mode === "demo" ? (
              <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-orange-500/15 text-orange-300 border border-orange-500/30 ml-1">MODE DÉMO</span>
            ) : (
              <span className={`text-[10px] font-mono px-2 py-0.5 rounded-full border ml-1 ${liveStatus === "ok" ? "bg-emerald-500/15 text-emerald-300 border-emerald-500/30" : liveStatus === "error" ? "bg-red-500/15 text-red-300 border-red-500/30" : "bg-slate-700/30 text-slate-300 border-slate-600/40"}`}>
                LIVE {liveStatus === "ok" && lastUpdated ? `· ${formatElapsed(Math.round((elapsedNow - lastUpdated) / 1000))}` : liveStatus === "loading" ? "· connexion…" : liveStatus === "error" ? "· indisponible" : ""}
              </span>
            )}
          </div>
          <div className="flex items-center gap-2">
            ...
            {mode === "demo" ? (
              <button onClick={() => setRunning((r) => !r)} className="text-xs px-3 py-1.5 rounded-full border border-slate-700 text-slate-300">{running ? "⏸ Pause" : "▶ Reprendre"}</button>
            ) : (
              <button onClick={() => { setLiveStatus("loading"); fetchLiveMatches().then((fresh) => { setMatches(fresh); setLiveStatus("ok"); setLastUpdated(Date.now()); }).catch((e) => { setLiveStatus("error"); setLiveError(e.message); }); }} className="text-xs px-3 py-1.5 rounded-full border border-slate-700 text-slate-300">⟳ Actualiser</button>
            )}
          </div>
        </div>
        <div className="max-w-5xl mx-auto px-4 pb-2 flex gap-1 overflow-x-auto">
          {nav.map(([key, label]) => (<button key={key} onClick={() => setView(key)} className={`text-xs px-3 py-1.5 rounded-lg whitespace-nowrap ${view === key || (view === "detail" && key === "live") ? "bg-slate-800 text-slate-100" : "text-slate-400"}`}>{label}</button>))}
        </div>
      </header>
      <main className="max-w-5xl mx-auto px-4 py-6">
        {mode === "live" && liveStatus === "error" && (
          <div className="bg-red-500/10 border border-red-500/30 rounded-2xl p-5 mb-6">
            ...
            <p className="text-xs text-slate-400">{liveError}. Aucune donnée simulée n'est utilisée en mode Live réel.</p>
          </div>
        )}
        {mode === "live" && liveStatus === "loading" && matches.length === 0 && (<div className="text-center text-slate-500 py-16 text-sm">Connexion…</div>)}
        {view === "dashboard" && <Dashboard matches={displayedMatches} pressures={pressures} filters={filters} setFilters={setFilters} onOpen={openMatch} />}
        {view === "live" && <Dashboard matches={displayedMatches} pressures={pressures} filters={filters} setFilters={setFilters} onOpen={openMatch} />}
        ...
        {view === "settings" && (<div className="space-y-8"><SettingsView weights={weights} setWeights={setWeights} /><AlertsPanel alerts={alerts} alertsEnabled={alertsEnabled} setAlertsEnabled={setAlertsEnabled} /></div>)}
